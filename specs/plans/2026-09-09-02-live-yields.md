...
Expected: PASS — 39 kernel tests, typecheck clean
...
export function normalizeMarket(raw: unknown, protocol: string, network: string): Market | null {
  ...
  if (typeof t.symbol !== "string" || typeof t.address !== "undefined") { /* shape guard below */ }
...
describe("getLendingMarkets", () => {
  ...
  it("skips a deployment returning GraphQL errors", async () => {
...
```json
    "test:live": "vitest run --testNamePattern='' packages/yields/test/live.integration.test.ts"
```
...
- [ ] `pnpm test` passes — 39 kernel, 19 obligations, 11 core, 28 yields (97 total)
